...
from reclab.recommenders import recommender
...
class KNNRecommender(recommender.PredictRecommender):
    ...
    def __init__(self, shrinkage=0, neighborhood_size=40, user_based=True, use_content=True, use_means=True, **kwargs):
    ...
    def dense_predictions(self):  # noqa: D102
        ...
        for idx in loop_range:
            relevant_idxs = nlargest_indices(self._neighborhood_size, self._similarity_matrix[idx])
    ...
    def update(self, users=None, items=None, ratings=None):  # noqa: D102
        ...
        self._means = divide_zero(flatten(self._feature_matrix.sum(axis=1)), self._feature_matrix.getnnz(axis=1))
        ...
        self._similarity_matrix = cosine_similarity(self._feature_matrix, self._feature_matrix, self._shrinkage)
        ...
        for user_id, item_id, _ in user_item:
            ...
            if self._use_means:
                if len(ratings) == 0:
                    ...
                    preds.append(mean + np.average(ratings - relevant_means[nonzero], weights=similarities))
            else:
                if len(ratings) == 0:
                    preds.append(0)
                else:
...
def cosine_similarity(X, Y, shrinkage):
    ...
    return divide_zero(
        (X @ Y.T).A,
        scipy.sparse.linalg.norm(X, axis=1)[:, np.newaxis] * scipy.sparse.linalg.norm(Y, axis=1)[np.newaxis, :]
        + shrinkage,
    )
...
def nlargest_indices(n, iterable):
    ...
    """
    nlargest = heapq.nlargest(n, enumerate(iterable), key=lambda x: x[1])
