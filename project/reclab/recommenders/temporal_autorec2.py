...
class TemporalAutoRecLib2(TemporalAutoRecLib):
    def __init__(
        ...
        super().__init__(
            num_users, num_items, temporal_window_size, seen_users, seen_items, hidden_neuron, dropout, random_seed
        )
...
class TemporalAutorec2(TemporalAutorec):
    def __init__(
        ...
        if recs_init_mode == "randint":
            self.recommendations = torch.randint(
                0, num_items, size=(num_users, temporal_window_size), dtype=torch.float32
            )
        else:
    ...
    def update(self, users=None, items=None, ratings=None):  # noqa: D102
        ...
        self.mask_recommendation = torch.FloatTensor(curr_recommendation.T).clamp(
            0, 1
        )  # NOTE: we could use the same mask as mask_ratings. (manorz, 02/19/22)
