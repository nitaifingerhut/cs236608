...
class LibFM(recommender.PredictRecommender):
    ...
    def __init__(
        ...
        **kwargs
        ...
        self._num_features = (
            self._max_num_users + num_user_features + self._max_num_items + num_item_features + num_rating_features
        )
        self._model = wpyfm.PyFM(
            method=method,
        ...
        if ratings is not None:
            ...
            for row, ((user_id_outer, item_id_outer), (rating, rating_context)) in enumerate(ratings.items()):
                ...
                for i, feature in enumerate(item_features):
                    row_col[0].append(row)
                    row_col[1].append(self._max_num_users + len(user_features) + self._max_num_items + i)
                    data.append(feature)
                for i, feature in enumerate(rating_context):
                    row_col[0].append(row)
                    row_col[1].append(
                        self._max_num_users + len(user_features) + self._max_num_items + len(item_features) + i
                    )
                    data.append(feature)

                new_rating_outputs.append(rating)

            new_rating_inputs = scipy.sparse.csr_matrix((data, row_col), shape=(len(ratings), self._num_features))
    ...
    def _predict(self, user_item):  # noqa: D102
        ...
        test_inputs = scipy.sparse.csr_matrix((data, row_col), shape=(len(user_item), self._num_features))
