...
from reclab.environments import environment
...
class Topics(environment.DictEnvironment):
    ...
    def _get_rating(self, user_id, item_id):  # noqa: D102
        topic = self._item_topics[item_id]
        rating = (
            self._user_preferences[user_id, topic]
    ...
    def _rate_items(self, user_id, item_ids):  # noqa: D102
        ...
        recommended[topic] = 1
        self._satiations = (
            self._satiation_decay * (self._satiations + recommended)
    ...
    def _reset_state(self):  # noqa: D102
        if self._user_bias_type == "normal":
            self._user_biases = self._init_random.normal(loc=0.0, scale=0.5, size=self._num_users)
        ...
        if self._item_bias_type == "normal":
            self._item_biases = self._init_random.normal(loc=0.0, scale=0.5, size=self._num_items)
        ...
        self._user_preferences = self._init_random.uniform(low=0.5, high=5.5, size=(self._num_users, self._num_topics))
        self._item_topics = self._init_random.choice(self._num_topics, size=self._num_items)
        self._users = collections.OrderedDict((user_id, np.zeros(0)) for user_id in range(self._num_users))
        self._items = collections.OrderedDict((item_id, np.zeros(0)) for item_id in range(self._num_items))
    ...
    def _update_state(self):  # noqa: D102
        if (self._timestep + 1) % self._shift_steps == 0:
            ...
            shifted_users = self._dynamics_random.choice(self._num_users, int(self._num_users * self._shift_frequency))
            new_preferences = self._init_random.uniform(low=0.5, high=5.5, size=(len(shifted_users), self._num_topics))
            if self._user_bias_type == "normal":
            ...
            self._user_preferences[shifted_users] = (
                self._shift_weight * self._user_preferences[shifted_users] + (1 - self._shift_weight) * new_preferences
            )
