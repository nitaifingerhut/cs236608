...
class DictEnvironment(Environment):
    ...
    def reset(self):
        ...
        if isinstance(self._initial_sampling, str):
            ...
            if self._initial_sampling == "uniform":
                ...
                item_probs = scipy.stats.beta.pdf(item_idx, a=0.75431, b=3.22225, loc=-1, scale=num_items + 1)
    ...
    def _get_user_prob(self):
        ...
        if dist_choice == "uniform":
            ...
            # Sample according to a powerlaw-like beta distribution,
            # parameters were fit to MovieLens 100k.
            user_dist = scipy.stats.beta.pdf(idx, a=0.70384, b=1.83271, loc=-1, scale=num_users + 1)
        else:
    ...
    def _select_online_users(self):
        ...
        return self._dynamics_random.choice(user_ids, size=num_online, replace=False, p=self._user_prob)
